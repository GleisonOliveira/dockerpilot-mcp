...
describe("StopContainersTool", () => {
  ...
  describe("stop all (no filter)", () => {
    ...
    it("uses default timeout of 10 when not specified", async () => {
      ...
      await capturedCallback({ dryRun: false });
    ...
    it("uses custom timeout when specified", async () => {
      ...
      await capturedCallback({ dryRun: false, timeout: 30 });
  ...
  describe("filter by names", () => {
    it("stops only containers matching name", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["web"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("stops multiple containers matching different names", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["web", "db"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("filter by ids", () => {
    it("stops container matching id prefix", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, ids: ["aaa111"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("stops multiple containers by id", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, ids: ["aaa111", "ddd444"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("names + ids combined", () => {
    it("matches container if it matches name OR id", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["db"], ids: ["ggg777"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("exclude", () => {
    it("excludes container by name", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, exclude: ["db"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("excludes container by short id", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, exclude: ["aaa111bbb222"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("excludes multiple containers", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, exclude: ["web", "cache"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("force=true", () => {
    it("calls kill instead of stop", async () => {
      ...
      await capturedCallback({ dryRun: false, force: true });
    ...
    it("reports stopped=true on successful kill", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, force: true }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("empty results", () => {
    ...
    it("returns empty results when filter matches nothing", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["nonexistent"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
  ...
  describe("stopDependents=true", () => {
    it("also stops containers that depend on the target via Compose depends_on", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["db"], stopDependents: true }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("does not stop dependents when stopDependents is false", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["db"], stopDependents: false }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
    ...
    it("does not include excluded containers even if they are dependents", async () => {
      ...
      const result = await capturedCallback({ dryRun: false, names: ["db"], stopDependents: true, exclude: ["web"] }) as { content: { text: string }[] };
      const parsed = JSON.parse(result.content[0].text);
